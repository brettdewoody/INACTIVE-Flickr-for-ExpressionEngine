...
class Eehive_flickr_ft extends EE_Fieldtype {
	 ...
	 *
	 */
	function _display_field($data, $flickrField) {
		...
		$r .= '<div class="flickrChooser" id="' . $flickrField . '_chooser" style="' . $displayChooser . '">
			<a  class="flickrInput" href="' . $this->helper->browser_url($flickrField) . '" onClick="return false;"><button >Choose Photo</button></a> No photo chosen
		</div>';
		$r .= '<div class="flickrImage" id="' . $flickrField . '_image" style="' . $displayImage . '">
			...
			<a class="flickrTrash" href="#" rel="' . $flickrField . '" onClick="return false;"><img src="' . $this->helper->theme_url() . 'images/trash-icon.gif" /></a>
		</div>';
		$r .= '</div>';
	...
	function replace_link($data, $params = array(), $tagdata = FALSE) {
		...
		$settings = $this->helper->cache['settings'];
